...
<img src="./views.svg" width="620" alt="Profile views"/>
...
* **Telemetry Graphics (`stats.svg`, `streak.svg`, `langs.svg`, `views.svg`)**: Custom Python scripts ([`make_stats.py`](scripts/make_stats.py), [`make_streak.py`](scripts/make_streak.py), [`make_langs.py`](scripts/make_langs.py), [`make_views.py`](scripts/make_views.py)) query GitHub's GraphQL and Traffic APIs to compute contribution streaks, commit counts, language distribution, and cumulative profile views. Language logos ([`lang_icons.py`](scripts/lang_icons.py)) are vendored at build time from [Simple Icons](https://simpleicons.org) (CC0) and embedded directly as path data—no runtime calls to any icon CDN.
* **Contact icons (`social-*.svg`)**: Generated by [`scripts/make_social.py`](scripts/make_social.py) as small vintage-panel chips wrapped in real markdown links—LinkedIn and ORCID use vendored Simple Icons brand marks, email and Lattes use generic stroke glyphs.
* **`views.svg`**: The GitHub Traffic API only reports the last 14 days of daily views, so [`make_views.py`](scripts/make_views.py) merges each day's count into [`views_state.json`](views_state.json) and sums it into a running total, avoiding any third-party counter service.
