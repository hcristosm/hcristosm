...
def fetch_languages_data():
    if not TOKEN:
        ...
        # Dados fallback caso execute sem token localmente
...
def generate_langs_svg(bytes_data, repos_data, output_path="langs.svg"):
  ...
  <!-- COLUNA 1: BY BYTES -->
  <text x="20" y="20" class="header">BY BYTES</text>
"""
    ...
    for i, (lang, size) in enumerate(sorted_bytes):
        ...
        bar_max_w = 140
        bar_w = max(4, int((pct / 100) * bar_max_w))

        svg_content += f"""  <text x="20" y="{y}" class="label">{lang}</text>
  <rect x="110" y="{y - 9}" width="{bar_max_w}" height="8" class="bar-bg"/>
  <rect x="110" y="{y - 9}" width="{bar_w}" height="8" class="bar-fill"/>
  <text x="260" y="{y}" class="value">{pct}%</text>
"""

    # Renderiza Coluna 2 (BY REPOS)
    svg_content += f"""\n  <!-- COLUNA 2: BY REPOS -->
  <text x="330" y="20" class="header">BY REPOS</text>
"""

    for i, (lang, count) in enumerate(sorted_repos):
        y = 42 + i * row_height
        bar_max_w = 140
        bar_w = max(4, int((count / max_repos) * bar_max_w))

        svg_content += f"""  <text x="330" y="{y}" class="label">{lang}</text>
  <rect x="420" y="{y - 9}" width="{bar_max_w}" height="8" class="bar-bg"/>
  <rect x="420" y="{y - 9}" width="{bar_w}" height="8" class="bar-fill"/>
  <text x="570" y="{y}" class="value">{count}</text>
"""
